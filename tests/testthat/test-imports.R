...
#   # files
#   file1 <- system.file("inst/extdata/", "sumobrain_autonomous_search1.xlsx", package="patentr")
#   file2 <- system.file("inst/extdata/", "sumobrain_autonomous_search2.xlsx", package="patentr")
#   files <- list(file1, file2)
#   # read it in 
#   patData <- importPatentData(files)
#   
...
test_that("can tolerate xls(x) that underreports number of columns",{
  # tidyverse test
  #df <- readxl::read_excel(rprojroot::find_testthat_root_file("testData","mtcars.xls"))
  #expect_identical(ncol(df),ncol(mtcars))
  # test modified
  # note, test cases modified to remove dependency on 0.1.1.9000 readxl, future versions (0.1.2+)
  # expected to fix this issue
  df2 <- readxl::read_excel(rprojroot::find_testthat_root_file("testData","sumobrain_autonomous_search1.xlsx"), skip=1)
